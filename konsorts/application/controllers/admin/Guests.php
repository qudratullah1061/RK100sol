...
class Guests extends Admin_Controller {
    ...
    public function get_guest_profile($user_id) {
//        if (!$user_id || ($user_id == 1 && $this->admin_info['admin_id'] != 1)) {
//            redirect(base_url('admin/dashboard'));
//        }
        $this->selected_tab = 'guest';
        $this->selected_child_tab = 'view';
        $member_info = $this->Members_Model->get_member_by_id($user_id);
        if ($member_info) {
            $data['member_info'] = $member_info;
            $data['country_options'] = GetCountriesOption($member_info['country']);
            $data['state_options'] = GetStatesOption($member_info['country']);
            $data['city_options'] = GetCityOptions($member_info['state']);
